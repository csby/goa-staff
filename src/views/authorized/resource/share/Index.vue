<template>
  <el-container>
    <el-aside width="420px">
      <catalog name="共享目录"
               :uri="this.$uris.adShareList"
               :add-uri="this.$uris.adShareAdd"
               add-label-width="120px"
               add-description-label="描述信息"
               add-street-label="备注信息"
               add-name-placeholder="Public Share"
               add-description-placeholder="总共容量：20.00GB"
               :add-street-placeholder="'访问地址:\n\\\\192.168.123.101\\Public Share'"
    ...
    <el-main>
      <div v-if="info.data">
        ...
        <div class="info" :style="infoStyle">
          <span class="detail">{{info.data.description}}</span>
          <pre class="detail">{{info.data.street}}</pre>
          <div class="group"
...
class Index extends RouterPage {
  ...
  filterCatalog (value, data) {
    ...
    if (data.street.indexOf(value) !== -1) {
      return true
    }
